...
import { SAMPLE_TAP_AND_GO_CARDS, KIGALI_BUS_STOPS, KIGALI_ROUTES } from '../data/kigaliTransitData';
...
export default function TapAndGoWidget() {
  ...
  const handleTopUp = () => {
    ...
    setTimeout(() => {
      setCards((prev) =>
        prev.map((c, i) => (i === activeCardIndex ? { ...c, balanceRwf: c.balanceRwf + amount } : c))
      );
    ...
    setCards((prev) =>
      prev.map((c, i) =>
        i === activeCardIndex
          ? {
              ...c,
              balanceRwf: c.balanceRwf - computedFareRwf,
              lastTappedStop: `${originStop.name} ➔ ${destStop.name}`,
              lastTappedTime: 'Just now',
              tripsToday: c.tripsToday + 1,
            }
          : c
      )
    );
    ...
    <div className="flex flex-col h-full bg-slate-900/95 backdrop-blur-md rounded-2xl border border-slate-800 shadow-xl overflow-hidden">
      ...
      <div className="p-4 border-b border-slate-800 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="text-base font-bold text-slate-100">Tap&Go Smart Card</h2>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5">
        ...
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-tr from-cyan-950 via-slate-900 to-blue-900 p-5 border border-cyan-500/30 shadow-2xl space-y-4">
          ...
          <div className="flex items-center justify-between pt-2">
            <div className="space-y-0.5">
              ...
              <div className="text-xs font-bold text-slate-100">{currentCard.holderName}</div>
            </div>
          ...
          <div className="flex items-center justify-between pt-1 border-t border-cyan-500/20 text-[10px] text-slate-300">
            <span>Last tapped: <strong className="text-white">{currentCard.lastTappedStop || 'Downtown Terminal'}</strong></span>
        ...
        <div className="flex items-center gap-2 text-xs">
          <span className="text-slate-400 font-medium">Active Card:</span>
          {cards.map((card, idx) => (
            <button
              key={card.cardNumber}
